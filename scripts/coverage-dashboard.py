...
MILESTONES = [
    ...
    ("M3", "Ordinary Matrix federation", "In progress",
     "The identity, transport, join, state, history, catch-up and fork-rig "
     "tracks are complete (#14–#16). Synapse interoperability runs nightly "
     "in both directions, and `federation_fork.rs` checks stale, disjoint, "
     "contested and partition-and-heal forks through real federation. The "
     "remaining frontier is the intermittent restricted-room join in #342; "
     "its suspected stale-copy mechanism was disproved and its faithful "
     "local sequence is now a permanent regression test."),
    ("M4", "Ecosystem integration", "**Done**",
     "Delegated MAS/OIDC authentication and the built-in OIDC provider are "
     "complete (#17). The application-service surface is complete (#18), "
     "with matrix-hookshot and the mautrix-go harness running in scheduled "
     "CI. Evidence and operator guidance live under `docs/evidence/` and "
    ...
    ("M7", "MatrixRTC", "In progress",
     ...
     "timeline arrives beside it. MSC4354 sticky events, which MSC4143 now"
     " makes `m.rtc.member`, are served: "
     "`org.matrix.msc4354.sticky_duration_ms` on `/send` and `/state` (and"
     " on a delayed send, so the pair Element Call drives works together),"
     " the `msc4354_sticky` key on the PDU outside the redaction algorithm"
     " and capped at the hour the MSC allows, expiry from the earlier of "
     "the event's own timestamp and its arrival so a peer cannot stretch "
     "it, an index keyed by expiry so what has lapsed is a bounded prefix,"
     " and the `msc4354_sticky` section of `/sync` -- everything unexpired"
     " on an initial sync or a join, only what is new since the token "
     "otherwise, never an event the timeline beside it already carries, "
     "each stamped with the time it has left. Across federation a sticky "
     "event rides `/send` like any PDU, and a server that joins later gets"
     " the room's unexpired ones both pushed through the outbox, as the "
     "MSC says, and carried in the `send_join` response, because a Spindle"
     " seeds the join response and backfills nothing, so a pushed event "
     "older than what it knows would have no predecessor to hang on. "
     "`crates/spindle-server/tests/sticky_events.rs` pins all of it, the "
     "three-server case included. What #40 still owes: the "
     "5/20/100-participant churn benchmarks. "
     ...
     "(scripts/element-call-check.py), run nightly and on demand. Thirteen"
     " of their specs pass and are protected: the landing page and "
     "sign-up; a guest creating a call, sharing the link and a second "
     "guest joining with video tiles on both sides; create-call with a "
     "clean leave; the JWT error paths; the two SFU-reconnect specs; and "
     "spa-call-sticky, which is MatrixRTC 2.0 -- this server's sticky "
     "events driven by the real client -- including the rejoin after an "
     "improper leave that is the kill scenario #41 names. That is LiveKit,"
     " lk-jwt-service and this server composing in a real browser, which "
     "is the composition #41 says no unit test reaches. The widget and "
     "restricted-SFU specs register users through Synapse's admin API with"
     " a shared secret, which this server does not serve, and stay out of "
